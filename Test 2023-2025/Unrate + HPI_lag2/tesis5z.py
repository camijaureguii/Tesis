...
# S1/S2/S3: UNRATE + UNRATE_yoy | S3: UNRATE solo si no significativo
# Train: 2007-2022 | Test: 2023-2025
...
features_full = {
    "S1_current": ["UNRATE", "HPI_gr_lag2"],
    "S2_30dpd":   ["UNRATE", "HPI_gr_lag2"],
    "S3_60dpd":   ["UNRATE", "HPI_gr_lag2"],
}
features_por_grupo = {
    "S1_current": ["UNRATE", "HPI_gr_lag2"],
    "S2_30dpd":   ["UNRATE", "HPI_gr_lag2"],
    "S3_60dpd":   ["UNRATE"],              # HPI_gr_lag2 p=0.695, no significativo
}
...
for grupo in grupos_no_default:
    ...
    train_g = df_g[df_g["anio"].between(2007, 2022)].dropna(subset=features_full_g + ["pd_anual_obs"]).copy()
    test_g  = df_g[df_g["anio"].between(2023, 2025)].dropna(subset=features).copy()
...
print("\n" + "="*60)
print("PD PREDICHA POR GRUPO Y AÑO (test 2023-2025)")  # Train: 2007-2022
print("="*60)
...
macro_anual[macro_anual["anio"].isin(anios_test)].to_parquet(
    os.path.join(path_archivos, "macro_obs_2023_2025.parquet"), index=False
)
