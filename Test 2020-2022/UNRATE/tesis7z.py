...
# Matriz 2019 (comparación estática pre-COVID)
P_2019 = construir_matriz(panel_cohort, [2019])

print("Matriz 2019 (comparación estática):")
...
# Para predecir 1 año adelante (2020): P^1
# Para predecir 2 años adelante (2021): P^2
# Para predecir 3 años adelante (2022): P^3
...
for anio in [2020, 2021, 2022]:
    pd_2019_h = P_2019["S4_default"]
    pd_proy_h = matrices_pit[anio]["S4_default"]

    print(f"\n{'─'*95}")
    print(f"VALIDACION {anio}")
    print(f"{'─'*95}")
    print(f"{'Grupo':<18} {'Observada':>10} {'Proyectada':>12} {'MAE':>8} {'Solo 2019':>12} {'MAE':>8}")
    print("-"*75)
    ...
    for g in grupos_no_default:
        ...
        proy  = pd_proy_h[g]
        s2019 = pd_2019_h[g]
        print(f"{g:<18} {obs:>10.6f} {proy:>12.6f} {abs(proy-obs):>8.6f} {s2019:>12.6f} {abs(s2019-obs):>8.6f}")
...
modelos_cols = {"Proyectada": "proyectada", "Solo 2019": "solo_2019"}

metricas = []
for anio_m in [2020, 2021, 2022]:
...
# Proyectada: P_2020 × P_2021 × P_2022
P_proy_3 = pd.DataFrame(
    matrices_pit[2020].values @ matrices_pit[2021].values @ matrices_pit[2022].values,
    index=estados, columns=estados
)

# Solo 2019: P_2019^3
P_2019_3 = potencia_matriz(P_2019, 3)

# PD observada a 3 años: M_obs_2020 x M_obs_2021 x M_obs_2022
M_obs_3 = pd.DataFrame(
    mat_obs[2020].values @ mat_obs[2021].values @ mat_obs[2022].values,
    index=estados, columns=estados
)
pd_obs_3 = M_obs_3["S4_default"].to_dict()

print(f"\n{'Grupo':<18} {'Observada':>10} {'Proyectada':>12} {'Solo 2019':>12}")
print("-"*60)
...
for g in grupos_no_default:
    ...
    s2019 = P_2019_3.loc[g, "S4_default"]
    print(f"{g:<18} {obs:>10.6f} {proy:>12.6f} {s2019:>12.6f}")
    resultados_3.append({
        "grupo":      g,
        "observada":  obs,
        "proyectada": proy,
...
P_2019_3.to_csv(os.path.join(path_archivos, "matriz_largo_plazo_2019.csv"))
